<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET")
    {
        ...
        // для ajax запроса из формы - зделано здесь что б не делать лишний запрос в базу данных с того файла - ну и для разнооборазия подходов
        if (!empty($_GET["sell_symbol"]))
        {
            $sell_symbol = $_GET["sell_symbol"];
            
            foreach ($own_shares as $own_share)
            {
                if ($own_share['symbol'] == $sell_symbol )
                {
                    print $own_share["shares"] ;
                    exit();
                }
            }
    ...
    else if ($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        else if (isset($_POST["sell_all"]))
        {
            // render form
            redirect("sell_all.php");
        }
    }
?>
